...
function blockText(blocks: readonly unknown[]): string {
  ...
  for (const block of blocks) {
    ...
    else if (b.type === 'tool-call') parts.push(`[tool-call ${String(b.name || '?')}] ${prettyMaybeJson(String(b.arguments || ''))}`)
...
export function apply(ctx: Context, config: Config) {
  ctx.effect(() => {
    ...
    const userLabel = () => useColor
      ...
      : '你 ›'
    const kimiLabel = () => paint('green', paint('bold', 'kimi ›'))
    const prompt = () => write(paint('brightCyan', 'kimi> '))
    ...
    write(`${paint('green', 'kimi-ignition ready')} ${paint('dim', `(agent: ${config.sessionId})`)}\n`)
    prompt()
