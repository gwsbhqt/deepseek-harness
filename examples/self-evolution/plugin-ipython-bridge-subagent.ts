...
function stringArg(value: unknown, fallback?: string): string {
  ...
  if (typeof value !== 'string') throw new Error('内核 host 桥参数必须是字符串')
  return value
}
...
export function apply(ctx: Context): void {
  ...
  const callerAgent = (kernel: string): Agent => {
    const agent = ctx.agents.get(SessionId(kernel))
  ...
  ctx.effect(() => {
    const disposers = [
      ctx.ipython.registerHostMethod('subagent_spawn', async (args, kernel) => {
        ...
        const label = stringArg(args[0], 'kernel-spawned worker')
        const started = await ctx.subagents.startContinuable({
          provider: 'spawn',
          label,
          request: {
            prompt: [{ type: 'text', text: stringArg(args[1], '') }],
            parent,
      ...
      ctx.ipython.registerHostMethod('subagent_interrupt', async (args, kernel) => {
        ...
        const parent = callerAgent(kernel)
        ctx.subagents.interrupt(SessionId(stringArg(args[0])), { kind: 'ancestor', agent: parent })
        return true
      }),
      ctx.ipython.registerHostMethod('subagent_report', async (args, kernel) => {
        ...
        return await ctx.subagents.reportFrom(
          child,
          [{ type: 'text', text: stringArg(args[0], '') }],
      ...
      ctx.ipython.registerHostMethod('subagent_send', async (args, kernel) => {
        ...
        return await ctx.subagents.followup(
          ...
          [{ type: 'text', text: stringArg(args[1], '') }],
