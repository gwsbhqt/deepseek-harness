...
export class DisposableList<T extends WeakKey> {
  ...
  // Node.js 的 console.log / util.inspect 会认这个 Symbol 方法，
  // 让它在调试输出里显示成普通数组的样子
...
export function isConstructor(func: any): func is new (...args: any) => any {
  ...
  // 生成器函数（或定义残缺的函数）也不能 new
  // 下面这种更严格的判断不能用，因为 mock.fn() 返回的函数是代理：
  // if (func.prototype.constructor !== func) return false
  if (func instanceof GeneratorFunction) return false
  // 有的环境把 AsyncGeneratorFunction polyfill 成了 Function 本身，此时这个判断会误伤，先排除
...
function handleError(info: StackInfo, reason: any, getOuterStack: () => string[]): never {
  ...
  // reason 连像样的堆栈都没有（比如抛了个普通值）：
  // 包成新 Error，堆栈整体换成外层堆栈
