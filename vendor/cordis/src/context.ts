...
export class Context {
  ...
  /**
   * 判断一个值是不是 Cordis 上下文（代理实例或上下文原型都算）。
   *
   * 品牌标记挂在一个全局 symbol 上而不是靠 `instanceof`，所以跨 realm
   * （不同的 JS 全局环境）、甚至项目里同时装了多份 cordis 也能正确识别。
   *
   * @param value — 待检查的值。
   * @returns `value` 是 Cordis 上下文时为 `true`，并把类型收窄为 Context。
   */
  static is(value: any): value is Context {
    return !!value?.[Context.is as any]
  }

  // 打上"跨副本品牌标记"。把函数 Context.is 用作属性键时（见上面的
  // `value?.[Context.is as any]`），JS 会先通过它的 [Symbol.toPrimitive]
  // 把函数转成键——这里转成全局注册的 Symbol.for('cordis.is')。第二行再把
  // 这个全局 symbol 标记打到 Context.prototype 上。于是任何一份 cordis 副
  // 本算出来的键都是同一个全局 symbol，跨副本、跨 realm 都能互相认出来。
  static {
  ...
  // Node.js 的 console.log / util.inspect 会认这个全局 symbol 方法：
  // 打印上下文时只显示 `Context <名字>`，避免把巨大的代理对象整个展开。
