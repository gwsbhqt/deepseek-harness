...
// 判断是否为 AggregateError（一个错误里打包了多个错误）：
// 不靠全局类名，只看是不是 Error 且带 errors 数组，兼容各种 polyfill
...
export class Logger {
  ...
  /**
   * 生成一个级别方法。每次调用时：
   * 1. 对单个 Error 参数做展开——带 cause 就递归记录 cause，
   *    是 AggregateError 就把里面每个错误拆成一条记录；
   * 2. 给消息编一个全服务递增的序号、打上时间戳；
   * 3. 分发给所有导出器，各自按级别阈值决定收不收（阈值查找顺序：
